...
import { FOOTER_SOCIAL_LINKS, useFooterTranslatedData } from "@/constants"

export default function Footer(){
    const {FOOTER_LINKS, FOOTER_COPYRIGHT_TEXT} = useFooterTranslatedData()
    return (
            ...
            <div className="flex flex-row flex-wrap justify-center h-full">
                {
                    FOOTER_LINKS.map((footer_link, index)=> (
                        <div key={index} className="flex flex-row justify-start mr-4">
                            <Link className="leading-4 mb-4 font-bold no-underline text-white" href={footer_link.link}>{footer_link.title}</Link>
                        </div>
                    ))
                }
            </div>
