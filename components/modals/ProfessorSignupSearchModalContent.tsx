...
import { PROFESSOR_SIGN_UP_MODAL_NAME, PROFESSOR_SIGN_UP_SEARCH_MODAL_NAME } from "@/constants";
import { useModal } from "@/hooks";
import ProfessorSignUpModalForm from "../forms/modelForm/ProfessorSignUpModalForm";
...
export default function ProfessorSignUpSearchModalContent() {
  ...
  const { open: openProfessorSignUpModal } = useModal(PROFESSOR_SIGN_UP_MODAL_NAME);
  ...
  const { data: searchResults, error, isLoading } = useSearchProfessorsQuery(
    searchQuery,
    {skip: !searchQuery.trim(),}, // Only run query if searchQuery is not empty
  );
                ...
                <div className="w-full max-w[545px] my-2">
                  ...
                  {searchQuery && searchResults && (
                    <ul className="absolute bg-white border border-gray-300 w-full max-h-60 overflow-y-auto mt-1 rounded-md z-10">
                      {searchResults.map((professor: any) => (
                        <li
                          key={professor.id}
                          className="p-2 cursor-pointer hover:bg-blue-500 hover:text-white"
                          onClick={() => {handleProfessorSignUp(professor)}}
                        >
